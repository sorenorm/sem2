...
author: "Astrid Olling, Daniel Christensen, Karoline Klitgaard, Søren Orm, Pernille Jensen"
date: "3/2/2020"
...
#linear regression on reaction time by days of sleep deprivation on our participant
...
The reaction times in the task were significantly effected by days of sleep deprevation with F(1, 8) = 23.32 at p-value = 0.001307
...
The coefficients for the regression are found in the table above.
...
The t-values, p-values, and degrees of freedom for each participant are found in the table above.
...
When looking at the p-values from the output it seems that sleep deprivation has a significant effect on reaction time for all participants. It should hovever be emphasized that one of the participants (no. 335) seems to have decreased reaction time by days of sleep deprivation. 
...
The slopes were found to be significantly greater than 0 with t(17) = 6.7715 at p > 0.001
...
```{r}
#sleepstudy$reaction_noise <- jitter(sleepstudy$Reaction ) 
#wuuuut

sleepstudy$noice <- runif(180, -10, 10)

sleepstudy$reaction_noiced <- sleepstudy$Reaction + sleepstudy$Reaction * sleepstudy$noice/100
